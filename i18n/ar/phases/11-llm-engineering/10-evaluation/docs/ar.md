...
### بنية تقييم ثلاث طبقات

في الهندسة العملية لذكاء الذكاء الاصطناعي، تستخدم أنظمة التقييم عادةً**layered architecture**لتحذير المخرجات من الأسفل إلى الأعلى. هذا التصميم يضمن سرعة وتوصيل التقييم مع السيطرة على تكاليف API وتحقيق التوازن بين الحكم النهائي والخبراء البشريين.
...
#### الطبقة 1: القاعدة القائمة على القواعد والتحديد (قاعدة القواعد)  يجب أن تفشل بسرعة

قبل أن تدعو قضاة LLM مكلفين وبطيئين، استخدم رمز تحديدية لتصفية الأخطاء البسيطة / الواضحة أولا.

- **Core Metrics for Implementation**:
  - **JSON-Schema Validation**: إذا كانت عملك تتطلب الامتثال الصارم لنموذج JSON محدد (على سبيل المثال، التسمية، استخراج البيانات المعدنية) ، فتحليل وتؤكيد ذلك مباشرة باستخدام الرمز. إذا فشل التحليل، قم بتسجيل صفر على الفور وتفوت التقييمات اللاحقة لـ LLM-as-a-judge.
  - **Regex & Sensitive Word Interception**: تحقق من وجود محفظة مكان في المخرج (مثل `{{missing_value}}`() تسرب النظام على الفور، أو تحتوي على طابع غير قانوني/حساس.
  - **Length & Type Constraints**: على سبيل المثال، إذا كان ملخص الفيلم محدودًا بـ 100 كلمة، قم بتشخيص عدم الامتثال إذا تجاوز 150 كلمة.
- **Best Practice**: دمج هذه الطبقة في اختبارات الوحدة أو بوابات CI. يستغرق ذلك فقط ميل ثانية، ويتمثل في صفر تكلفة API، ويعمل كصمام سلامة فعال للغاية من حيث التكلفة.

#### الطبقة 2: ماجستير في مجال القانون كقاضي  الحركة الحالية في الصناعة

استخدم نموذج أكثر قدرة (توصى بالقاعدة 3.5 Sonnet أو GPT-4o كقاضي) لتقييم النموذج المستهدف.

- **Three Main Evolution Directions**:
  1. **G-Eval (Rubric-based Natural Language Scoring)**: إعط القضاة إشارة مفصلة للغاية وأجبرها على استخدامها**Chain-of-Thought (CoT)**(اكتب الأسباب أولاً، وقطع في النهاية)
  2. **QAG Mode (Question-Answer Generation Loop)**: مناسبة بشكل خاص للسيناريوهات RAG (الإنعاش المُتزايد). بدلاً من الحصول على درجات مباشرة، اطرح على القاضي أسئلة ثنائية: "هل الجواب مبني بالكامل على وثائق المرجعية؟ (نعم/لا) " ، "هل يجري الإجابة على سؤال المستخدم؟ (نعم/لا) " ، تحويل الأسئلة الذاتية إلى مهام متعددة الخيارات الموضوعية / ثنائية يمنح نتائج مستقرة للغاية.
  3. **Pairwise Preference (Head-to-Head)**: إعطاء نتائج من الإشارة القديمة والإشارة الجديدة جنبا إلى جنب إلى جنب إلى القاضي (المجهول لاختبار العمى) ودع القاضي يختار ما هو أفضل (A يفوز / B يفوز / التساوي).
- **Preventing Judge Biases**:
  > [!تحذير]
  > **The Three Sins of LLM Judges:**
  > - **Verbosity Bias**: القاضي يميل إلى تفضيل الردود أطول وأكثر الكلمات.
  > - **Position Bias**: في اختبار العمى، يحيز القاضي نحو اختيار الخيار A (الموقع الأول).
  > - **Self-Enhancement Bias**: نموذجات GPT تفضل نتائج في نمط GPT، في حين أن نموذجات كلود تفضل نتائج في نمط كلود.
  
  **Countermeasures**:
  - في المشاركة، قل صراحة: "الوضوح هو مقياس مهم لتقييم. لا تقدم درجات عالية للردود المفصلة أو الطويلة للغاية".
  - للمقارنات المتزاوجة، قم بتبادل مواقف A و B وإجراء التقييم مرة أخرى، وتأخذ متوسط النتيجة.
  - اختر نماذج القضاة من عائلة مختلفة عن النموذج الذي يتم تقييمه (على سبيل المثال، استخدم كلود لتقييم GPT).
  - استخدم التصنيف الثنائي (مر / فشل ، نعم / لا) بدلاً من مقياس 1-5 Likert حيثما أمكن. إنخراطات الثنائية أصعب التضخم وتعد أكثر استقرارًا للتكاثر.

#### الطبقة الثالثة: التقييم البشري

لا يُقصد التقييم البشري لتقييم آلاف البيانات يومياً، والتي ستفقد أي فريق. الغرض الوحيد هو**calibrating the judge model (Aligning the Judge)**. . .

- **Core Implementation Practices**:
  1. **Blind Testing (Arena Mode)**: اختر 3050 حالة صعبة ومشوشة من التدفقات التجارية الأساسية وطلب من الخبراء البشريين تصنيفها بشكل أعمى.
  2. **Consistency Check**: حساب معايير الاتفاق (مثل كابا كوهين) بين درجات البشر ودرجات القاضي LLM.
- **Best Practice**:
  - إذا كان التوافق بين قاضي ماجستير في العلوم القانونية والخبراء البشريين يصل إلى 80٪85٪ +، يعتبر القاضي موثوقًا في الهندسة. يمكنك بعد ذلك تحميل 95٪ من اختبارات التراجع الآلي بثقة إلى هذا القاضي.
  - عندما يحدث خطأ في التنظيم، تحقق من "الاعتبارات" التي قدمها القاضي وتحسين نظام النموذج القاضي في الحال وفقا لذلك.
...
### مجموعة بيانات Eval: البناء وأفضل الممارسات

نوعية مجموعة بيانات التقييم الخاصة بك تحدد مباشرة سقف نتائج التقييم الخاصة بك. في بناء مجموعة بيانات التقييم، هناك ثلاثة أنواع من الحالات التجارية التالية ضرورية لتشكيل خط أساسي كامل للجودة:
...
إن بناء مجموعة بيانات تقييم عالية الجودة يتطلب خطوات منهجية والامتثال لأفضل الممارسات.

#### 1. أربعة خطوات أساسية لبناء مجموعة بيانات التقييم

1. **Define Evaluation Goals & Scenarios (Defining Schema)**
   لا تحاول بناء مجموعة بيانات "وحيدة الحجم" ، أولاً ، حدد السيناريوهات التي يتم تطبيق نموذجك عليها:
   - **General Capabilities**: التفكير، التشفير، اللعب الدوري، تلخيص النص، إلخ.
   - **Vertical Business**: تطابق الفيلم، استخراج البيانات المعدنية الآلية، تصحيح الخطأ ASR، فهم الفيديو الطويل، إلخ.
   - **Guardrail Evaluation**: السلامة، معدل الهلوسة، الصمود (حساسية على الأخطاء الطباعة والضوضاء).

2. **Determine Data Format (Data Format)**
   JSONL هو النموذج الأكثر استخداماً في الصناعة. كل سطر هو كائن JSON مستقل ، وهو مناسب للتدفق. اعتماداً على طريقة التقييم ، يتم هيكلة شكل البيانات عموماً على النحو التالي:
...
3. **Data Sourcing & Collection (Data Sourcing)**
   - **Production Logs (Highly Recommended)**: أخذ عينات من حركة الإنتاج الفعلية (تعليقات المستخدم) وتحديدها عن اسمها. هذا يمثل "اختبارًا حقيقيًا" أكثر واقعية لعملك.
   - **Synthetic Data**: استخدم النماذج الأكثر قدرة (مثل GPT-4o أو Claude 3.5 Sonnet) لتوليد الإشارات على شكل دفعة وفقاً لخطط محددة، ثم تصفيها عن طريق مراجعة البشر أو التفاصيل.
   - **Adoption/Modification of Public Benchmarks**: تعديل المعايير القياسية مثل MMLU أو GSM8K أو MATH لتتناسب مع متطلبات عملك المحددة.

4. **Cleaning & Validation (Data Curation)**
   - **Deduplication**: استخدام خوارزميات التشابه التشعري أو التطابق الدقيق لمنع ظهور الإشارات المتطابقة تقريبًا عدة مرات.
   - **Difficulty Grading**: تصنيف مجموعة البيانات إلى سهلة / متوسطة / صعبة للتمييز الواضح في مستويات الأداء بين النماذج المختلفة.
   - **Rule-based Filtering**: فلترا من الأخطاء النحوية، والعبارات الغامضة، والإشارات التي لا تحددها جيدا، أو المعلومات الحساسة.

#### 2. أفضل الممارسات لبناء مجموعة بيانات

1. **Independence & Preventing Cheating (Data Contamination Prevention)**
   - لا تسمح أبداً بتسريب عينات مجموعة بيانات التقييم إلى تدريب النموذج أو مجموعة بيانات التنظيم الدقيق.
   - إذا كانت مجموعات التقييم مشتقة من مجموعات بيانات عامة، قم بتحديثها أو إعادة كتابتها بشكل دوري (على سبيل المثال، تبادل الكيانات، تغيير الأرقام، تغيير هيكل الجملة) لمنع النماذج من الحصول على درجات عالية من خلال التذاكر.

2. **Coverage & Balanced Distribution (Data Distribution)**
   - **Scenario Coverage**: إذا كانت حالة الاستخدام الخاصة بك تتضمن "فهم محتوى الفيديو"، يجب أن تتضمن مجموعة بيانات التقييم ليس فقط طلبات عامة طويلة النص، ولكن أيضا حالات معقدة مع طوابع زمنية، أو البيانات المعدنية المتعددة، أو استفسارات سياق طويلة.
   - **Tag Balance**: تأكد من أن توزيع الفئات (مثل التجميع والإستخراج والحكم والترجمة) يتوافق مع حركة الإنتاج الفعلية.

3. **Building the "Golden Dataset" (Golden Dataset)**
   - لا تتبع حجم مجموعة البيانات بشكل أعمى. بالنسبة لتقييمات الأعمال، فإن "مجموعة البيانات الذهبية" عالية الجودة والتي يتم التحقق منها يدويا من 100500 حالة هي أكثر قيمة بكثير من 10،000 نقطة بيانات اصطناعية غير نظيفة.
   - مجموعة بيانات ذهبية تمكن من اختبار التراجعة السريع، مما يسمح لك بتشغيل التقييمات في دقائق وتحديد التذبذب الجودة عند ضبط أو تحديث النموذج الأساسي.

4. **Incorporating Adversarial Testing (Adversarial Testing)**
   - أدخل عينات معارضة بنسبة 5٪10٪ في مجموعة البيانات.
   - أمثلة: المدخلات التي تحتوي على أخطاء كتابة، أو السياق المتناقض، أو الإشارات المصممة لإثارة الأخطاء (الحقن العاجلة). هذا أمر حاسم لتقييم قوة النموذج وحدود الأمن.

5. **Version Control (Versioning)**
   - يجب أن تتكرر مجموعات البيانات مثل الكود. تنفيذ التحكم في الإصدارات الواضحة (مثل ، `eval_v1.0.jsonl`،`eval_v1.1_hard.jsonl`)
   - المزامنة وتحديث مجموعة التقييم في كل مرة تتغير فيها منطق الأعمال التجارية عبر الإنترنت أو أنماط سلوك المستخدم.

### حجم العينات والثقة

50 حالة اختبار ليست كافية

إذا كان تقديرك يصل إلى 90% على 50 حالة (أي 45 صحيحة من 50) ، ففترة الائتمان 95% التي يتم حسابها باستخدام طريقة الثنائي الدقيق (كلوبر-بيرسون) هي **[78%, 97%]**وهذا يعني أن الدقة الحقيقية لديها فرصة بنسبة 95% للنخفض بين 78٪ و 97٪، وفرق 19 نقطة مئوية.

في 200 حالة مع دقة 90٪، تتضيق فترة الائتمان إلى **[85%, 94%]**الآن يمكنك اتخاذ القرارات
