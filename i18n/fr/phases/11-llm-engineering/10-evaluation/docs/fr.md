...
### Architecture d'évaluation à trois couches

Dans l'ingénierie pratique de l'IA, les systèmes d'évaluation adoptent généralement une approche**layered architecture**Cette conception assure le débit et la vitesse d'évaluation tout en contrôlant les coûts de l'API et en alignant le jugement final avec les experts humains.
...
#### Couche 1: Réglementation et détermination  Fail-Fast

Avant d'appeler des juges de LLM coûteux et lents, utilisez le code déterministe pour filtrer d'abord les erreurs simples/obvies.

- **Core Metrics for Implementation**- Le numéro de la liste:
  - **JSON-Schema Validation**Si votre entreprise exige un strict respect d'un format JSON spécifique (par exemple, l'étiquetage, l'extraction de métadonnées), analysez-le et validez-le directement en utilisant du code. Si le partage échoue, marquez-le immédiatement à zéro et sautez les évaluations ultérieures de la LLM-as-a-judge.
  - **Regex & Sensitive Word Interception**: Vérifiez si la sortie contient des placeholders (comme `{{missing_value}}`), fuit le système immédiatement ou contient des caractères illégaux/sensitifs.
  - **Length & Type Constraints**: Par exemple, si un résumé d'un film est limité à 100 mots, marquez-le comme non conforme s'il dépasse 150 mots.
- **Best Practice**Cette couche est intégrée dans des tests d'unité ou des portes CI. Elle ne prend que quelques millisecondes, ne coûte rien à l'API et sert de vanne de sécurité très rentable.

#### Couche 2: LLM-as-a-Judge  Le cheval de travail de l'industrie actuelle

Utiliser un modèle plus performant (la clause 3.5 Sonnet ou GPT-4o sont recommandés comme juges) pour évaluer le modèle cible.

- **Three Main Evolution Directions**- Le numéro de la liste:
  1. **G-Eval (Rubric-based Natural Language Scoring)**: Donner au juge une rubrique extrêmement détaillée et l'obliger à l'utiliser **Chain-of-Thought (CoT)**(écrivez d'abord les raisons, marquez à la fin).
  2. **QAG Mode (Question-Answer Generation Loop)**: particulièrement adapté aux scénarios RAG (Retrieval-Augmented Generation). Au lieu de donner un score direct, posez au juge des questions binaires: "La réponse est-elle entièrement basée sur des documents de référence? (oui/non) ", "La réponse répond-elle à la question de l'utilisateur? (oui/non)".
  3. **Pairwise Preference (Head-to-Head)**: Donnez les sorties de l'ancien prompt et du nouveau prompt côte à côte au juge (anonymisé pour les tests à l' aveugle) et laissez le juge choisir lequel est meilleur (A gagne / B gagne / Tie).
- **Preventing Judge Biases**- Le numéro de la liste:
  > Je suis désolé.
  > **The Three Sins of LLM Judges:**
  > - **Verbosity Bias**Le juge a tendance à favoriser des réponses plus longues et plus émouvantes.
  > - **Position Bias**: Dans les tests à l'aveugle, le juge est partial en choisissant l'option A (placé en premier).
  > - **Self-Enhancement Bias**: Les modèles GPT préfèrent les sorties de style GPT, tandis que les modèles Claude préfèrent les sorties de style Claude.
  
  **Countermeasures**- Le numéro de la liste:
  - Dans le message, indiquez explicitement: " La concission est une mesure clé de l'évaluation.
  - Pour les comparaisons par paires, échangez les positions de A et B et faites à nouveau l'évaluation, en prenant le score moyen.
  - Choisissez des modèles de juge d'une famille différente du modèle évalué (par exemple, utilisez Claude pour évaluer le GPT).
  - Utilisez une classification binaire (Pass/Fail, Oui/Non) au lieu d'une échelle de 1 à 5 Likert lorsque cela est possible.

#### Couche 3: Évaluation humaine  Ancrage de calibration

L'évaluation humaine n'est pas destinée à évaluer des milliers de points de données par jour, ce qui épuisera toute équipe.**calibrating the judge model (Aligning the Judge)**- Je suis désolé .

- **Core Implementation Practices**- Le numéro de la liste:
  1. **Blind Testing (Arena Mode)**: Sélectionnez 3050 cas difficiles et ambiguës dans les flux d'affaires principaux et faites en sorte que les experts humains les évaluent aveuglément.
  2. **Consistency Check**: Compute les mesures d'accord (comme la Kappa de Cohen) entre les notes humaines et les notes du juge LLM.
- **Best Practice**- Le numéro de la liste:
  - Si la cohérence entre le juge de la LLM et les experts humains atteint 80%85%+, le juge est considéré comme fiable en ingénierie.
  - Lorsque des erreurs surviennent, examinez le "réflexion" fourni par le juge et affiner le système du modèle du juge en conséquence.
...
### Eval Datasets: Construction et meilleures pratiques

La qualité de votre ensemble de données d'évaluation détermine directement le plafond des résultats de votre évaluation.
...
La mise en place d'un ensemble de données d'évaluation de haute qualité exige des mesures systématiques et le respect des meilleures pratiques.

#### 1. Quatre étapes essentielles pour créer un ensemble de données d'évaluation

1. **Define Evaluation Goals & Scenarios (Defining Schema)**
   Ne cherchez pas à créer un ensemble de données "unique pour tous".
   - **General Capabilities**: Réflexion, codage, jeu de rôle, résumé du texte, etc.
   - **Vertical Business**: correspondance de films, extraction automatique de métadonnées, correction des erreurs ASR, compréhension de longues vidéos, etc.
   - **Guardrail Evaluation**: Sécurité, taux d'hallucination, robustesse (sensibilité aux fautes de frappe et au bruit).

2. **Determine Data Format (Data Format)**
   JSONL est le format le plus utilisé dans l'industrie. Chaque ligne est un objet JSON indépendant, qui est pratique pour le streaming.
   - **Objective Evaluation (with standard reference answers)**- Le numéro de la liste:
     ```json
     {"question": "Who directed The Matrix?", "answer": "The Wachowskis", "category": "movie_trivia"}
     ```
   - **Subjective/Open-ended Evaluation (based on LLM-as-a-Judge or human evaluation)**- Le numéro de la liste:
     ```json
...
3. **Data Sourcing & Collection (Data Sourcing)**
   - **Production Logs (Highly Recommended)**: Prenez des échantillons du trafic de production réel (User Prompts) et anonymez-le.
   - **Synthetic Data**: Utilisez les modèles les plus performants (par exemple, GPT-4o ou Claude 3.5 Sonnet) pour générer des requêtes en lots selon un schéma spécifique, puis filtrer par l'intermédiaire d'une révision humaine ou d'une heuristique.
   - **Adoption/Modification of Public Benchmarks**: Adaptez les critères de référence standard tels que MMLU, GSM8K ou MATH pour répondre à vos besoins spécifiques.

4. **Cleaning & Validation (Data Curation)**
   - **Deduplication**: Utilisez des algorithmes de similitude sémantique ou une correspondance exacte pour empêcher que des Prompts presque identiques apparaissent plusieurs fois.
   - **Difficulty Grading**: Catégorisez le jeu de données en Easy / Medium / Hard pour différencier clairement les niveaux de performance entre les différents modèles.
   - **Rule-based Filtering**: Éliminer les erreurs grammaticales, les phrases ambiguës, les indications mal définies ou les informations sensibles.

#### 2. Les meilleures pratiques pour la construction de données

1. **Independence & Preventing Cheating (Data Contamination Prevention)**
   - Ne jamais laisser les échantillons de données d'évaluation fuir dans votre formation de modèle ou vos ensembles de données de réglage.
   - Si les ensembles d'évaluation sont dérivés de ensembles de données publics, les mettre à jour ou les réécrire périodiquement (par exemple, échanger des entités, changer des chiffres, modifier les structures de phrases) pour empêcher les modèles de marquer haut purement par mémorisation.

2. **Coverage & Balanced Distribution (Data Distribution)**
   - **Scenario Coverage**Si votre cas d'utilisation implique la "compréhension du contenu vidéo", votre ensemble de données d'évaluation ne devrait pas seulement inclure des requêtes générales de texte long, mais aussi des cas complexes avec des timestamps, des métadonnées hétérogènes ou des requêtes de contexte long.
   - **Tag Balance**: Assurez-vous que la répartition des catégories (par exemple, résumé, extraction, raisonnement, traduction) s'aligne sur votre trafic de production réel.

3. **Building the "Golden Dataset" (Golden Dataset)**
   - Ne poursuivez pas aveuglément la taille des ensembles de données. Pour les évaluations d'entreprise, un "ensemble de données d'or" de haute qualité, vérifié manuellement de 100 à 500 cas, est beaucoup plus précieux que 10 000 points de données synthétiques non nettoyés.
   - Un ensemble de données doré permet de tester rapidement la régression, ce qui vous permet d'exécuter des évaluations en quelques minutes et de détecter les dérives de qualité lors de l'ajustement fin ou de la mise à niveau du modèle de base.

4. **Incorporating Adversarial Testing (Adversarial Testing)**
   - Introduire des échantillons de 5% à 10% de l'adversité dans le jeu de données.
   - Exemples: des entrées avec des fautes de frappe, un contexte contradictoire ou des requêtes conçues pour induire des erreurs (Injections de prompt).

5. **Version Control (Versioning)**
   - Les ensembles de données, comme le code, doivent être itérés.`eval_v1.0.jsonl`- Je suis là .`eval_v1.1_hard.jsonl`)
   - Synchroniser et mettre à jour le jeu d'évaluation chaque fois que la logique des affaires en ligne ou les comportements des utilisateurs changent.

### Taille de l'échantillon et confiance

50 cas d'essai ne suffisent pas.

Si votre évaluation donne un score de 90% sur 50 cas (c'est-à-dire 45 corrects sur 50), l'intervalle de confiance de 95% calculé à l'aide de la méthode binaire exacte (Clopper-Pearson) est **[78%, 97%]**- la précision réelle est de 95%, soit un écart de 19 points de pourcentage.

Dans 200 cas avec une précision de 90%, l' intervalle de confiance se resserre à **[85%, 94%]**Maintenant, vous pouvez prendre des décisions.
