...
**BM25**(Best Matching 25) standart anahtar kelime arama algoritmasıdır. 1990'lardan beri arama motorlarının (Elasticsearch ve Lucene gibi) omurgası olmuştur. Klasik TF-IDF algoritmasına evrimsel bir yükseltme.

BM25'in nasıl çalıştığını hızlı bir şekilde kavramsal olarak anlamak istiyorsanız,**three core pillars**(Matematik formüller tarafından sıkılmadan):

1. **Term Rarity (IDF: The rarer the term, the higher its weight)**- ...
   - Bir sorguda, "ne", "ne" gibi ortak terimler neredeyse her belgede görünür ve doğru belgeyi tanımlamaya yardımcı olmaz. Öte yandan, "E-4021" (özel bir hata kodu) veya "halüsinasyon" (özel bir konu) gibi nadir terimler son derece seçicidir. BM25 otomatik olarak nadir terimlere ve indirimlere yüksek bir ağırlık verir veya yaygın terimleri görmezden gelir.
2. **Term Frequency Saturation (TF Saturation: Repeated hits have diminishing returns, with a ceiling)**- ...
   - Temel fikir "kilit kelime dolumu" önlemektir. Bir terimi 1 kez belirtmek yararlıdır, 2 kez ise biraz daha yararlıdır, ancak 100 kez belirtmek bir belgeyi 100 kat daha önemli kılmaz. BM25 terim frekans katkı (parametre ile kontrol edilen) için bir tavan belirler.$k_1$) Bir terim bir belgede daha çok kez görünce, artışlı puan katkısı hızla doymuş ve platolar.
3. **Document Length Normalization (L-Norm: Matching in short documents is more significant)**- ...
   - Temel fikir şişmiş, "suzlanmış" belgeleri cezalandırmaktır. 10 kelimelik kısa bir başlık bir anahtar kelimeyle eşleşirse ve 10.000 kelimelik kapsamlı beyaz kağıt bir kez aynı anahtar kelimeyle eşleşirse, kısa başlık tamamen bu anahtar kelimeye adanmış olma olasılığı yüksek. BM25, göreceli uzunluğuna göre belgeleri cezalandırır (parametre ile kontrol edilir $b$), daha kısa belgelerin artması ve son derece uzun belgelerin puanlarının düşürülmesi.

> - Ne?
> Detaylı matematiksel formüller, Dövüşler Arası'nın düzeltme türevleri, parametreler kontrolleri ve adım adım el ile hesaplanan sayısal örnek için özel sayfaya bakın: [BM25 Mathematical Details & Code Alignment](bm25_math_en.md)- Evet .


### Karşılıklı Rank Füzyonu (RRF)

Büyük ölçekli RAG veya Hibrit Arama mimarlıklarında, **RRF (Reciprocal Rank Fusion)**sıralama listelerini birleştirmek için en yaygın olarak kabul edilen ve zarif algoritmadır.

Basitçe söylemek gerekirse, RRF'nin temel mantığı: **disregard raw scores, focus only on ranks**- Evet .

#### 1. Neden RRF'ye ihtiyacımız var?
Hibrit bir geri alma sistemi oluştururken, genellikle kullanıyoruz:
- **Semantic Retrieval (Vector Search)**: Notlar genellikle cosine benzerliği veya nokta ürünüdür, genellikle $[-1, 1]$veya $[0, 1]$- Evet .
- **Keyword Retrieval (BM25)**: Notlar sınırsız pozitif yüzen nokta sayılardır (sırın sıklığına ve belge uzunluğuna bağlıdır).

Bu iki puanın doğrudan toplamı matematiksel olarak yanlışdır çünkü ölçekleri, dağılımları ve fiziksel anlamları tamamen farklıdır. RRF, bu uyum sorunu sadece "her bir listedeki bir belgenin göreci konumunu" odaklayarak, farklı geri alım sistemlerinin çıkışlarını tek boyutlara standartlaştırarak atlıyor.

#### 2. Formula'ya derinlemesine dal

Başvuru belgesi için $d$, RRF puanı aşağıdaki şekilde hesaplanır:
