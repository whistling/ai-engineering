...
### Üç katman değerlendirme mimarisi

Pratik bir AI mühendisliği alanında, değerlendirme sistemleri tipik olarak bir **layered architecture**Bu tasarım, değerlendirme geçiş ve hızını sağlayarak API maliyetlerini kontrol ederek ve son yargılamayı insan uzmanlarına uyumlu hale getirir.
...
#### Katman 1: Kurallara dayalı ve belirleyici (Kurallara dayalı)  Hasarlı-Hızlı

Pahalı ve yavaş LLM yargıçları çağırmadan önce önce basit/açık hatalı filtrelemek için belirleyici kod kullanın.

- **Core Metrics for Implementation**- ...
  - **JSON-Schema Validation**Şirketiniz belirli bir JSON biçimine (örneğin etiketleme, metadata çıkarımı) sıkı bir şekilde uymayı gerektirirse, doğrudan kod kullanarak analiz edin ve doğrulayın.
  - **Regex & Sensitive Word Interception**: Çıktırma yer tutucuları içermiğini kontrol edin (örneğin `{{missing_value}}`), sistemde bir sızıntı çıkartıyor veya yasadışı/hissel karakterler içerir.
  - **Length & Type Constraints**Örneğin, bir filmin özetinin 100 kelime ile sınırlı olması durumunda, 150 kelime üzerinde ise, bu filmi uygun olmayan bir film olarak işaretleyin.
- **Best Practice**Bu katman birim testlerine veya CI kapılarına yerleştirilmelidir. Sadece milisaniyeler sürer, sıfır API maliyeti vardır ve yüksek maliyetli bir güvenlik valf olarak hizmet eder.

#### Katman 2: Yargıç olarak LLM  Şu anki endüstri iş atı

Hedef modeli değerlendirmek için daha yetenekli bir model kullanın (Klavü 3.5 Sonnet veya GPT-4o yargıç olarak önerilmektedir).

- **Three Main Evolution Directions**- ...
  1. **G-Eval (Rubric-based Natural Language Scoring)**: Yargıçın çok detaylı bir kuralını ver ve onu kullanmaya zorla**Chain-of-Thought (CoT)**(Önce nedenleri yazın, sonunda puan verin).
  2. **QAG Mode (Question-Answer Generation Loop)**: RAG (Retrieval-Augmented Generation) senaryoları için özellikle uygundur. Doğrudan puanlama yerine yargıçtan ikili sorular sor: "Çevabı tamamen referans belgeleri üzerine mi dayanır? (Evet/Hayır) " "Çevabı kullanıcının sorusuna cevap veriyor mu? (Evet/Hayır) "Süjetik soruları objektif çoklu seçim/ikili görevlere dönüştürmek son derece istikrarlı sonuçlar verir.
  3. **Pairwise Preference (Head-to-Head)**: Eski ve yeni istekleri yan yana hakimlere gönderin (kör test için anonimleştirilmiş) ve hakim daha iyi olanı seçsin (A kazan / B kazan / eşitlik).
- **Preventing Judge Biases**- ...
  > - Ne? - Hayır.
  > **The Three Sins of LLM Judges:**
  > - **Verbosity Bias**Yargıç daha uzun ve daha kelimeci cevapları tercih eder.
  > - **Position Bias**: Kör testlerde yargıç A seçeneğini seçmek için tarafsızdır (birincisi yerleştirilmiştir).
  > - **Self-Enhancement Bias**: GPT modelleri GPT tarzında çıkışları tercih ederken Claude modelleri Claude tarzında çıkışları tercih eder.
  
  **Countermeasures**- ...
  - Bu sorunun cevabını açıkça belirtin: "Konuçlu olmak değerlendirme için önemli bir ölçümdür.
  - Çiftlik karşılaştırmalar için A ve B'nin pozisyonlarını değiştirin ve değerlendirmeyi tekrar yaparak ortalama puan alın.
  - Değerlendirilmiş modelden farklı bir aileden yargıç modellerini seçin (örneğin, GPT değerlendirmesi için Claude kullanın).
  - Mümkünse 1-5 Likert ölçeği yerine ikili sınıflandırma (Gitme/Başarısız, Evet/Hayır) kullanın.

#### Katman 3: İnsan değerlendirme  Kalibrasyon Ankırı

İnsan değerlendirmesi, her ekibini yok edecek binlerce veri noktasını günde değerlendirmek için değil.**calibrating the judge model (Aligning the Judge)**- Evet .

- **Core Implementation Practices**- ...
  1. **Blind Testing (Arena Mode)**: Temel iş akışlarından 3050 zorlu, belirsiz vaka seçin ve insan uzmanlarının onları kör bir şekilde değerlendirmelerine izin verin.
  2. **Consistency Check**: İnsan notları ve LLM yargıç notları arasında hesaplama anlaşma ölçümleri (Cohen'in Kappa gibi).
- **Best Practice**- ...
  - LLM yargıçı ve insan uzmanları arasındaki tutarlılık %80 85%+'a ulaşırsa, yargıç mühendislik güvenilir olarak kabul edilir.
  - Yanlış bir uyum oluştuğunda, yargıç tarafından sağlanan "düşünme"yi inceleyin ve yargıç modelinin sistemini derhal uygun olarak düzeltin.
...
### Eval Veri Satıları: İnşaat ve En İyi Uygulamalar

Değerlendirme verilerinin kalitesi, değerlendirme sonuçlarının tavanını doğrudan belirler.Değerlendirme verilerinin oluşturulmasında, tam bir kalite temelini oluşturmak için aşağıdaki üç tür test vakaları gereklidir:
...
Yüksek kaliteli bir değerlendirme verisi oluşturmak sistematik adımlar ve en iyi uygulamalara uymayı gerektirir.

#### 1. Değerlendirme Verileri Oluşturmak İçin Dört Ana Adım

1. **Define Evaluation Goals & Scenarios (Defining Schema)**
   "Her şeye uygun bir boyut" veri kümesi oluşturmaya çalışmayın. Öncelikle modelinizin uygulanacağı senaryoları açıklayın:
   - **General Capabilities**: Dönüşüm, kodlama, rol oynama, metin özetleme vb.
   - **Vertical Business**: Film eşleşimi, otomatik metadata çıkarımı, ASR hata düzeltmesi, uzun video anlama vb.
   - **Guardrail Evaluation**: Güvenlik, Halüsinasyon oranı, Güçlülik (bu hataya ve gürültüye karşı hassasiyet).

2. **Determine Data Format (Data Format)**
   JSONL, endüstriste en yaygın kullanılan formatdır. Her satır bağımsız bir JSON nesnesi, akış için kullanışlıdır. Değerlendirme yöntemine bağlı olarak, veri biçimi genellikle şöyle yapılandırılır:
...
3. **Data Sourcing & Collection (Data Sourcing)**
   - **Production Logs (Highly Recommended)**: Gerçek üretim trafiğinden örnek alın (User Prompts) ve anonimleştirin. Bu işiniz için en gerçekçi "gerçek test" oluşturur.
   - **Synthetic Data**: Özel bir Şema'ya göre İpuçları serici olarak oluşturmak için en yetenekli modeller (örneğin, GPT-4o veya Claude 3.5 Sonnet) kullanın, sonra insan incelemesi veya heuristikleri ile filtreleyin.
   - **Adoption/Modification of Public Benchmarks**MMLU, GSM8K veya MATH gibi standart referansları özel iş gereksinimlerinize uygun hale getirin.

4. **Cleaning & Validation (Data Curation)**
   - **Deduplication**: Yaklaşık aynı Prompts'ın birden fazla kez görünmesini önlemek için semantik benzerlik algoritmaları veya tam eşleşmeyi kullanın.
   - **Difficulty Grading**: Veriler kümesini farklı modeller arasında performans seviyelerini açık bir şekilde ayırt etmek için kolay / orta / zor olarak sınıflandırın.
   - **Rule-based Filtering**: Grammatik hataları, belirsiz ifadeler, yanlış tanımlanmış ipuçları veya hassas bilgileri filtreleyin.

#### 2. Veritaşın İnşaatı için En İyi Uygulamalar

1. **Independence & Preventing Cheating (Data Contamination Prevention)**
   - Değerlendirme veri kümesi örneklerinin model eğitiminize veya ince ayarlama veri kümelerinize sızmasına asla izin vermeyin.
   - Değerlendirme kümeleri kamu verileri kümelerinden alınırsa, modellerin sadece bellekleme yoluyla yüksek puan almasını önlemek için onları düzenli olarak güncelleme veya yeniden yazın (örneğin, birimleri değiştirme, sayıları değiştirme, cümle yapısını değiştirme).

2. **Coverage & Balanced Distribution (Data Distribution)**
   - **Scenario Coverage**: Kullanım durumunuz "video içeriğini anlama" içerirse, değerlendirme verileriniz sadece genel uzun metin istekleri değil, aynı zamanda zaman damgaları, heterogen metadata veya uzun bağlamlı sorular ile karmaşık durumları da içermelidir.
   - **Tag Balance**Kategorilerin dağıtımı (örneğin özetleme, çıkarma, akıl yürütme, çevirme) gerçek üretim trafiğinize uyumlu olmasını sağlayın.

3. **Building the "Golden Dataset" (Golden Dataset)**
   - Verita setinin büyüklüğünü kör bir şekilde takip etmeyin. İşletme değerlendirmeleri için, 100 500 vakalardan oluşan yüksek kaliteli, elle doğrulanmış "Altın Verita" bir "Sintez veri kümesi" 10.000 temizlenmemiş sentetik veri noktasından çok daha değerlidir.
   - Altın Veriler Satısı hızlı Gerileme Testini sağlar ve temel modelin ince ayarlanmasında veya yükseltilmesinde değerlendirmeyi dakikalar içinde çalıştırmanıza ve kalite sürümlerini tespit etmenize olanak sağlar.

4. **Incorporating Adversarial Testing (Adversarial Testing)**
   - Veriler kümesine %5~10% karşıt örnekler girilsin.
   - Örnekler: yazı hatası, çelişkili bağlam veya hataları tetiklemek için tasarlanmış istekler (Sürekli Enjeksiyonlar).

5. **Version Control (Versioning)**
   - Kod gibi veri kümeleri tekrarlanmalıdır.`eval_v1.0.jsonl`- Evet .`eval_v1.1_hard.jsonl`)
   - Online iş mantığı veya kullanıcı davranış kalıpları değişirken değerlendirme seti senkronize edilmeli ve güncellenmelidir.

### Örnek Boyutu ve Güven

50 test vakası yeterli değil.

Eğer değerlendirmeniz 50 durumda %90 puan alırsa (yani 50'den 45'i doğru), tam ikili yöntemle (Clopper-Pearson) hesaplanan %95 güven aralığı **[78%, 97%]**Bu, gerçek doğruluk %78 ile %97 arasında düşme olasılığının %95 olduğunu, bu örnek boyutunda gerçek doğruluk %80 ile %96 arasındaki bir sistemin istatistiksel olarak fark edilmemesini gerektirir.

%90 doğrulukla 200 vakalarda güven aralığı **[85%, 94%]**Artık karar verebilirsin.
