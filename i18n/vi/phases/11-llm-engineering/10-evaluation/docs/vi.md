...
Trong kỹ thuật AI thực tế, các hệ thống đánh giá thường áp dụng một**layered architecture**để lọc các kết quả từ dưới lên. Thiết kế này đảm bảo thông qua đánh giá và tốc độ trong khi kiểm soát chi phí API và phù hợp phán quyết cuối cùng với các chuyên gia con người.
...
#### Lớp 1: dựa trên quy tắc & quyết định ( dựa trên quy tắc)  Phải thất bại nhanh chóng

Trước khi gọi các thẩm phán LLM đắt tiền và chậm, hãy sử dụng mã xác định để lọc ra những lỗi đơn giản / rõ ràng trước tiên.

- **Core Metrics for Implementation**- Có thể là:
  - **JSON-Schema Validation**Nếu doanh nghiệp của bạn yêu cầu tuân thủ nghiêm ngặt định dạng JSON cụ thể (ví dụ: thẻ, trích xuất siêu dữ liệu), phân tích và xác nhận trực tiếp bằng mã. Nếu phân tích thất bại, lập tức ghi số 0 và bỏ qua các đánh giá LLM như một thẩm phán tiếp theo.
  - **Regex & Sensitive Word Interception**: Kiểm tra xem sản phẩm có chứa các vị trí (như `{{missing_value}}`), rò rỉ hệ thống nhanh chóng, hoặc chứa các ký tự bất hợp pháp/ nhạy cảm.
  - **Length & Type Constraints**Ví dụ: Nếu bản tóm tắt phim chỉ có 100 từ, hãy đánh dấu nó không phù hợp nếu nó vượt quá 150 từ.
- **Best Practice**: Nhập lớp này vào các thử nghiệm đơn vị hoặc cổng CI. Nó chỉ mất vài millisecond, không phải chi phí API và hoạt động như một van an toàn hiệu quả về chi phí cao.

#### Lớp 2: LLM-as-a-Judge  Công nghiệp hiện tại

Sử dụng mô hình có khả năng hơn (Định vịnh 3.5 Sonnet hoặc GPT-4o được khuyến cáo như các thẩm phán) để đánh giá mô hình mục tiêu.

- **Three Main Evolution Directions**- Có thể là:
  1. **G-Eval (Rubric-based Natural Language Scoring)**: Đưa cho thẩm phán một quy tắc rất chi tiết và buộc nó sử dụng **Chain-of-Thought (CoT)**(đầu tiên viết lý do, ghi điểm ở cuối).
  2. **QAG Mode (Question-Answer Generation Loop)**: Đặc biệt phù hợp với các kịch bản RAG (Retrieval-Augmented Generation). Thay vì ghi điểm trực tiếp, hãy hỏi thẩm phán các câu hỏi nhị phân: "Phản ứng hoàn toàn dựa trên tài liệu tham khảo? (Có/Không)", "Phản ứng có giải quyết câu hỏi của người dùng không? (Có/Không)". Việc chuyển đổi các câu hỏi chủ quan thành các nhiệm vụ đa lựa chọn/nhị phân khách quan tạo ra kết quả rất ổn định.
  3. **Pairwise Preference (Head-to-Head)**: Đưa các kết quả từ các prompt cũ và mới bên cạnh với thẩm phán (được ẩn danh cho thử nghiệm mù) và để thẩm phán chọn ra nào tốt hơn (A thắng / B thắng / Tie).
- **Preventing Judge Biases**- Có thể là:
  > [Cảnh báo]
  > **The Three Sins of LLM Judges:**
  > - **Verbosity Bias**Thẩm phán có xu hướng ủng hộ những câu trả lời dài hơn, nhiều lời nói hơn.
  > - **Position Bias**Trong thử nghiệm mù, thẩm phán có thiên hướng chọn lựa A (được đặt đầu tiên).
  > - **Self-Enhancement Bias**: Các mô hình GPT thích các sản phẩm kiểu GPT, trong khi các mô hình Claude thích các sản phẩm kiểu Claude.
  
  **Countermeasures**- Có thể là:
  - Hãy nói rõ ràng trong lời nhắc: "Sự nói ngắn gọn là một thước đo đánh giá quan trọng.
  - Đối với so sánh đôi, thay đổi vị trí của A và B và chạy đánh giá lại, lấy điểm trung bình.
  - Chọn các mô hình thẩm phán từ một gia đình khác với mô hình được đánh giá (ví dụ, sử dụng Claude để đánh giá GPT).
  - Sử dụng phân loại nhị phân (Pass/Fail, Yes/No) thay vì thang điểm 1-5 Likert khi có thể.

#### Lớp 3: Đánh giá của con người  Phòng chuẩn

Đánh giá con người không phải là để đánh giá hàng ngàn điểm dữ liệu mỗi ngày mà sẽ đốt cháy bất kỳ đội nào.**calibrating the judge model (Aligning the Judge)**- Tôi không biết.

- **Core Implementation Practices**- Có thể là:
  1. **Blind Testing (Arena Mode)**: Chọn 3050 trường hợp khó khăn, mơ hồ từ dòng chảy kinh doanh cốt lõi và có các chuyên gia con người đánh giá chúng mù quáng.
  2. **Consistency Check**: tính toán các số liệu thỏa thuận (như Kappa của Cohen) giữa các điểm của con người và điểm của thẩm phán LLM.
- **Best Practice**- Có thể là:
  - Nếu sự nhất quán giữa thẩm phán LLM và các chuyên gia con người đạt đến 80%85%+, thẩm phán được coi là đáng tin cậy về kỹ thuật.
  - Khi xảy ra sự không phù hợp, kiểm tra "sự lý luận" do thẩm phán cung cấp và tinh chỉnh hệ thống mô hình thẩm phán ngay lập tức phù hợp.
...
### Eval Datasets: Xây dựng & Thực hành tốt nhất

Chất lượng của bộ dữ liệu đánh giá của bạn trực tiếp xác định giới hạn của kết quả đánh giá của bạn.
...
Việc xây dựng một bộ dữ liệu đánh giá chất lượng cao đòi hỏi phải có những bước có hệ thống và tuân thủ các thực tiễn tốt nhất.

#### 1. Bốn bước cơ bản để xây dựng một bộ dữ liệu đánh giá

1. **Define Evaluation Goals & Scenarios (Defining Schema)**
   Đừng cố gắng xây dựng một bộ dữ liệu "một kích thước phù hợp với tất cả". Đầu tiên, làm rõ các kịch bản mô hình của bạn được áp dụng cho:
   - **General Capabilities**: Lý luận, Coding, Role-play, Summary Text, vv
   - **Vertical Business**: Đáp ứng phim, lấy siêu dữ liệu tự động, sửa lỗi ASR, hiểu video dài, vv
   - **Guardrail Evaluation**: An toàn, Tốc độ ảo giác, Độ bền (sự nhạy cảm với sai lầm đánh chữ và tiếng ồn).

2. **Determine Data Format (Data Format)**
   JSONL là định dạng được sử dụng phổ biến nhất trong ngành công nghiệp. Mỗi dòng là một đối tượng JSON độc lập, thuận tiện cho phát trực tuyến. Tùy thuộc vào phương pháp đánh giá, định dạng dữ liệu thường được cấu trúc như sau:
   - **Objective Evaluation (with standard reference answers)**- Có thể là:
     ```json
     {"question": "Who directed The Matrix?", "answer": "The Wachowskis", "category": "movie_trivia"}
     ```
   - **Subjective/Open-ended Evaluation (based on LLM-as-a-Judge or human evaluation)**- Có thể là:
     ```json
...
3. **Data Sourcing & Collection (Data Sourcing)**
   - **Production Logs (Highly Recommended)**: lấy mẫu từ lưu lượng sản xuất thực tế (User Prompts) và làm cho nó vô danh.
   - **Synthetic Data**: Sử dụng các mô hình có khả năng nhất (ví dụ, GPT-4o hoặc Claude 3.5 Sonnet) để tạo hàng loạt Prompts theo một Schema cụ thể, sau đó lọc chúng thông qua đánh giá của con người hoặc heuristics.
   - **Adoption/Modification of Public Benchmarks**: Chuyển đổi các tiêu chuẩn chuẩn như MMLU, GSM8K hoặc MATH để phù hợp với các yêu cầu kinh doanh cụ thể của bạn.

4. **Cleaning & Validation (Data Curation)**
   - **Deduplication**: Sử dụng thuật toán tương đồng ngữ nghĩa hoặc phù hợp chính xác để ngăn chặn các Prompts gần giống nhau xuất hiện nhiều lần.
   - **Difficulty Grading**: Đánh phân nhóm bộ dữ liệu thành dễ dàng / trung bình / khó để phân biệt rõ ràng các mức độ hiệu suất trên các mô hình khác nhau.
   - **Rule-based Filtering**: Tránh ra các lỗi ngữ pháp, cách phác thảo không rõ ràng, những lời nhắc không được định nghĩa rõ ràng hoặc thông tin nhạy cảm.

#### 2. Các thực tiễn tốt nhất cho việc xây dựng bộ dữ liệu

1. **Independence & Preventing Cheating (Data Contamination Prevention)**
   - Đừng bao giờ cho phép các mẫu tập dữ liệu đánh giá rò rỉ vào các tập dữ liệu đào tạo mô hình hoặc điều chỉnh tinh tế của bạn.
   - Nếu các tập hợp đánh giá được lấy từ tập hợp dữ liệu công cộng, thường xuyên cập nhật hoặc viết lại chúng (ví dụ, trao đổi các thực thể, thay đổi số, thay đổi cấu trúc câu) để ngăn chặn các mô hình ghi điểm cao thông qua ghi nhớ.

2. **Coverage & Balanced Distribution (Data Distribution)**
   - **Scenario Coverage**: Nếu trường hợp sử dụng của bạn liên quan đến "sự hiểu biết nội dung video", bộ dữ liệu đánh giá của bạn không chỉ nên bao gồm các yêu cầu văn bản dài chung, mà còn các trường hợp phức tạp với dấu thời gian, metadata đa dạng hoặc các truy vấn ngữ cảnh dài.
   - **Tag Balance**: Đảm bảo rằng phân phối các loại (ví dụ: tổng kết, trích xuất, lý luận, dịch) phù hợp với lưu lượng sản xuất thực tế của bạn.

3. **Building the "Golden Dataset" (Golden Dataset)**
   - Đừng theo đuổi kích thước tập hợp dữ liệu một cách mù quáng. Đối với các đánh giá kinh doanh, một "Dữ liệu vàng" chất lượng cao, được xác minh bằng tay của 100500 trường hợp có giá trị hơn nhiều so với 10.000 điểm dữ liệu tổng hợp không được làm sạch.
   - Một bộ dữ liệu vàng cho phép kiểm tra hồi quy nhanh chóng, cho phép bạn chạy các đánh giá trong vài phút và phát hiện các biến động chất lượng khi điều chỉnh hoặc nâng cấp mô hình cơ bản.

4. **Incorporating Adversarial Testing (Adversarial Testing)**
   - Đưa 5%~10% mẫu đối kháng vào bộ dữ liệu.
   - Ví dụ: đầu vào có lỗi đánh chữ, bối cảnh mâu thuẫn hoặc các lời nhắc được thiết kế để gây ra lỗi (Prompt Injections).

5. **Version Control (Versioning)**
   - Các tập hợp dữ liệu, như mã, phải lặp lại. Thực hiện kiểm soát phiên bản rõ ràng (ví dụ: `eval_v1.0.jsonl`- `eval_v1.1_hard.jsonl`().
   - Đồng bộ hóa và cập nhật bộ đánh giá bất cứ khi nào logic kinh doanh trực tuyến hoặc mô hình hành vi của người dùng thay đổi.

### Số lượng mẫu và sự tự tin

50 trường hợp thử nghiệm không đủ.

Nếu đánh giá của bạn đạt điểm 90% trên 50 trường hợp (tức là, 45 chính xác trên 50), khoảng thời gian tin cậy 95% được tính bằng cách sử dụng phương pháp nhị phân chính xác (Clopper-Pearson) là **[78%, 97%]**Điều này có nghĩa là độ chính xác thực sự có khả năng 95% giảm từ 78% đến 97%, một sự lây lan 19 điểm phần trăm.

Trong 200 trường hợp với độ chính xác 90%, khoảng thời gian tin cậy bị chặt chẽ hơn đến **[85%, 94%]**Giờ anh có thể đưa ra quyết định.
