...
### Arquitetura de Avaliação de Três Camadas

Na engenharia prática da IA, os sistemas de avaliação tipicamente adotam uma**layered architecture**Este projeto garante a velocidade e o rendimento da avaliação, controlando simultaneamente os custos da API e alinhando o julgamento final com os peritos humanos.
...
#### Layer 1: Regras baseadas e deterministas (regras baseadas)  Deve falhar-rápido

Antes de chamar juízes de LLM caros e lentos, use o código determinista para filtrar erros simples/obvios primeiro.

- **Core Metrics for Implementation**- Não .
  - **JSON-Schema Validation**Se a sua empresa requer o cumprimento rigoroso de um formato JSON específico (por exemplo, tagging, extração de metadados), analise e valida-o diretamente usando código.
  - **Regex & Sensitive Word Interception**: Verifique se a saída contém possuidores de lugar (como `{{missing_value}}`), vai vazando o sistema imediatamente ou contém caracteres ilegais/sensiveis.
  - **Length & Type Constraints**Por exemplo, se um resumo de um filme for limitado a 100 palavras, marque-o como não-conformista se exceder 150 palavras.
- **Best Practice**Esta camada é incorporada em testes de unidade ou portais de CI.

#### Layer 2: LLM-as-a-Judge  O actual cavalo de trabalho da indústria

Utilize um modelo mais capaz (recomendam-se como juízes o clado 3.5 Sonnet ou o GPT-4o) para avaliar o modelo-alvo.

- **Three Main Evolution Directions**- Não .
  1. **G-Eval (Rubric-based Natural Language Scoring)**Fornecer ao juiz uma rubrica extremamente detalhada e forçá-la a usar **Chain-of-Thought (CoT)**(Escreva primeiro as razões, marque no final).
  2. **QAG Mode (Question-Answer Generation Loop)**A resposta é particularmente adequada para os cenários RAG (Retrieval-Augmented Generation). Em vez de obter pontos diretos, faça perguntas binárias ao juiz: "A resposta é totalmente baseada em documentos de referência? (Sim/Não) ", "A resposta aborda a pergunta do usuário? (Sim/Não)".
  3. **Pairwise Preference (Head-to-Head)**: Entregue as saídas do antigo e do novo prompt lado a lado ao juiz (anonimado para testes cegos) e deixe o juiz escolher qual é melhor (A vence / B vence / Tie).
- **Preventing Judge Biases**- Não .
  > - Não, não.
  > **The Three Sins of LLM Judges:**
  > - **Verbosity Bias**O juiz tende a favorecer respostas mais longas e mais verbais.
  > - **Position Bias**No teste cego, o juiz é tendencioso em escolher a opção A (posição em primeiro lugar).
  > - **Self-Enhancement Bias**: Os modelos GPT preferem as saídas de estilo GPT, enquanto os modelos Claude preferem as saídas de estilo Claude.
  
  **Countermeasures**- Não .
  - Explique explicitamente no aviso: "A concisa é uma métrica chave para avaliar.
  - Para comparações em pares, troque as posições de A e B e execute novamente a avaliação, tomando a pontuação média.
  - Escolha modelos de juízes de uma família diferente do modelo a ser avaliado (por exemplo, use Claude para avaliar o GPT).
  - Use classificação binária (Pass/Fail, Sim/Não) em vez de uma escala de 1-5 Likert quando possível.

#### Layer 3: Avaliação humana  Ancão de calibração

A avaliação humana não é destinada a classificar milhares de pontos de dados diários, o que acabaria com qualquer equipa.**calibrating the judge model (Aligning the Judge)**- Não .

- **Core Implementation Practices**- Não .
  1. **Blind Testing (Arena Mode)**: Selecione 3050 casos difíceis e ambíguos dos fluxos de negócios principais e faça com que os peritos humanos os avaliem cegamente.
  2. **Consistency Check**: Compute métricas de acordo (como a Kappa de Cohen) entre as notas humanas e as notas de juiz LLM.
- **Best Practice**- Não .
  - Se a consistência entre o juiz de LLM e especialistas humanos atinge 80%85%+, o juiz é considerado confiável em engenharia.
  - Quando ocorrer um desalinhamento, inspecione o "razão" fornecido pelo juiz e refine o sistema do modelo do juiz em conformidade.
...
### Eval Datasets: Construção e melhores práticas

A qualidade do seu conjunto de dados de avaliação determina diretamente o limite máximo dos resultados da sua avaliação.
...
A construção de um conjunto de dados de avaliação de alta qualidade requer medidas sistemáticas e a adesão às melhores práticas.

#### 1. Quatro passos essenciais para criar um conjunto de dados de avaliação

1. **Define Evaluation Goals & Scenarios (Defining Schema)**
   Não tente construir um conjunto de dados "de um tamanho único para todos".
   - **General Capabilities**Reasoning, Coding, Role-play, Summarização de textos, etc.
   - **Vertical Business**: film matching, extracção automática de metadados, correcção de erros ASR, compreensão de vídeos longos, etc.
   - **Guardrail Evaluation**: Segurança, taxa de alucinação, robustez (sensibilidade aos erros de digitação e ruído).

2. **Determine Data Format (Data Format)**
   JSONL é o formato mais usado na indústria. Cada linha é um objeto JSON independente, que é conveniente para streaming. Dependendo do método de avaliação, o formato de dados é geralmente estruturado como:
   - **Objective Evaluation (with standard reference answers)**- Não .
     ```json
     {"question": "Who directed The Matrix?", "answer": "The Wachowskis", "category": "movie_trivia"}
     ```
   - **Subjective/Open-ended Evaluation (based on LLM-as-a-Judge or human evaluation)**- Não .
     ```json
...
3. **Data Sourcing & Collection (Data Sourcing)**
   - **Production Logs (Highly Recommended)**A análise de dados e informações de dados é feita através de um sistema de informação e de dados de dados.
   - **Synthetic Data**: Utilize os modelos mais capazes (por exemplo, GPT-4o ou Claude 3.5 Sonnet) para gerar por lotes Prompts de acordo com um Esquema específico, e depois filtrá-los através de revisão humana ou heurísticas.
   - **Adoption/Modification of Public Benchmarks**Adaptar os referências padrão como MMLU, GSM8K ou MATH para se adequar às suas necessidades específicas de negócio.

4. **Cleaning & Validation (Data Curation)**
   - **Deduplication**: Use algoritmos semânticos de semelhança ou correspondência exata para evitar que Prompts quase idênticos apareçam várias vezes.
   - **Difficulty Grading**: Categorizar o conjunto de dados em Fácil / Médio / Difícil para diferenciar claramente os níveis de desempenho entre os diferentes modelos.
   - **Rule-based Filtering**: Elimine erros gramaticais, frases ambíguas, indicações mal definidas ou informações confidenciais.

#### 2. Melhores práticas para a construção de conjuntos de dados

1. **Independence & Preventing Cheating (Data Contamination Prevention)**
   - Nunca permita que as amostras dos conjuntos de dados de avaliação se infiltreem no seu treinamento de modelos ou em conjuntos de dados de ajuste fino.
   - Se os conjuntos de avaliação forem derivados de conjuntos de dados públicos, atualize-os ou reescreva-os periodicamente (por exemplo, trocando entidades, alterando números, alterando estruturas de frases) para evitar que os modelos obtenham pontuações altas puramente através da memorização.

2. **Coverage & Balanced Distribution (Data Distribution)**
   - **Scenario Coverage**Se o seu caso de utilização envolver "compreensão do conteúdo de vídeo", o seu conjunto de dados de avaliação deve incluir não apenas Prompts gerais de texto longo, mas também casos complexos com timestamps, metadados heterogêneos ou consultas de contexto longo.
   - **Tag Balance**A distribuição das categorias (por exemplo, resumo, extracção, raciocínio, tradução) deve estar alinhada com o seu tráfego de produção.

3. **Building the "Golden Dataset" (Golden Dataset)**
   - Para as avaliações de negócios, um "Datosset de ouro" de alta qualidade, verificado manualmente de 100 a 500 casos é muito mais valioso do que 10.000 pontos de dados sintéticos não limpos.
   - Um conjunto de dados dourado permite testes de regressão rápidos, permitindo executar avaliações em minutos e detectar desvios de qualidade ao ajustar ou atualizar o modelo base.

4. **Incorporating Adversarial Testing (Adversarial Testing)**
   - Introdução de amostras adversas de 5% a 10% no conjunto de dados.
   - Exemplos: entradas com erros de digitação, contexto conflitante ou instruções projetadas para induzir erros (Injeções de Injeção).

5. **Version Control (Versioning)**
   - Os conjuntos de dados, como o código, devem ser iterados. Implementar controle de versão claro (por exemplo, `eval_v1.0.jsonl`- Não .`eval_v1.1_hard.jsonl`)).
   - Sincronizar e atualizar o conjunto de avaliação sempre que a lógica de negócios on-line ou os padrões de comportamento do usuário mudem.

### Tamanho da amostra e confiança

50 casos de ensaio não são suficientes.

Se a sua avaliação tiver uma pontuação de 90% em 50 casos (ou seja, 45 correctos em 50), o intervalo de confiança de 95% calculado usando o método binomial exato (Clopper-Pearson) é **[78%, 97%]**O que significa que a precisão real tem 95% de probabilidade de cair entre 78% e 97%, uma diferença de 19 pontos percentuais.

Em 200 casos com uma precisão de 90%, o intervalo de confiança se aperta para **[85%, 94%]**Agora podes tomar decisões.
