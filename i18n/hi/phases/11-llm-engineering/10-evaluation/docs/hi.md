...
### तीन परतों की मूल्यांकन वास्तुकला

व्यावहारिक एआई इंजीनियरिंग में, मूल्यांकन प्रणाली आमतौर पर एक **layered architecture**इस डिजाइन से एपीआई लागतों को नियंत्रित करते हुए और मानव विशेषज्ञों के साथ अंतिम निर्णय को संरेखित करते हुए मूल्यांकन की गति और गति सुनिश्चित होती है।
...
#### स्तर 1: नियम आधारित और निर्धारक (नियम आधारित)  विफल-जलद होना चाहिए

महंगे और धीमे LLM न्यायाधीशों को बुलाने से पहले, पहले सरल/स्पष्ट त्रुटियों को फ़िल्टर करने के लिए निर्धारात्मक कोड का उपयोग करें।

- **Core Metrics for Implementation**:
  - **JSON-Schema Validation**: यदि आपके व्यवसाय को एक विशिष्ट JSON प्रारूप (जैसे, टैगिंग, मेटाडेटा निष्कर्षण) के साथ सख्त अनुपालन की आवश्यकता है, तो कोड का उपयोग करके इसे सीधे विश्लेषण और सत्यापित करें। यदि विश्लेषण विफल होता है, तो तुरंत इसे शून्य स्कोर करें और बाद में एलएलएम-ए-जजज मूल्यांकन को छोड़ दें।
  - **Regex & Sensitive Word Interception**: जांचें कि क्या आउटपुट में प्लेसहोल्डर हैं (जैसे `{{missing_value}}`), सिस्टम के शीघ्र लीक हो जाता है, या इसमें अवैध/संवेदनशील वर्ण होते हैं।
  - **Length & Type Constraints**: उदाहरण के लिए, यदि एक फिल्म का सारांश 100 शब्दों तक सीमित है, तो इसे 150 शब्दों से अधिक होने पर अनुपालन नहीं होने के रूप में चिह्नित करें।
- **Best Practice**: इस परत को यूनिट टेस्ट या आईसी गेट में एम्बेड करें. इसमें केवल मिलीसेकंड लगते हैं, इसमें शून्य एपीआई लागत होती है, और यह एक अत्यधिक लागत प्रभावी सुरक्षा वाल्व के रूप में कार्य करता है।

#### स्तर 2: LLM-as-a-Judge  वर्तमान उद्योग कार्यघोड़ा

लक्ष्य मॉडल का मूल्यांकन करने के लिए अधिक सक्षम मॉडल का उपयोग करें (कलॉड 3.5 सोनट या GPT-4o को न्यायाधीशों के रूप में अनुशंसित किया जाता है) ।

- **Three Main Evolution Directions**:
  1. **G-Eval (Rubric-based Natural Language Scoring)**: न्यायाधीश को एक अत्यंत विस्तृत rubric प्रदान करें और उसे उपयोग करने के लिए मजबूर करें **Chain-of-Thought (CoT)**(पहले कारण लिखें, अंत में स्कोर करें) ।
  2. **QAG Mode (Question-Answer Generation Loop)**: विशेष रूप से RAG (Retrieval-Augmented Generation) परिदृश्यों के लिए उपयुक्त। प्रत्यक्ष स्कोरिंग के बजाय, न्यायाधीश से द्विआधारी प्रश्न पूछेंः "क्या उत्तर पूरी तरह से संदर्भ दस्तावेजों पर आधारित है? (हाँ/नहीं) ", "क्या उत्तर उपयोगकर्ता के प्रश्न को संबोधित करता है? (हाँ/नहीं) " उद्देश्य बहुविकल्पीय/बाइनरी कार्यों में व्यक्तिगत प्रश्नों को परिवर्तित करने से अत्यधिक स्थिर परिणाम प्राप्त होते हैं।
  3. **Pairwise Preference (Head-to-Head)**: पुराने प्रॉम्प्ट और नए प्रॉम्प्ट के आउटपुट को एक साथ न्यायाधीश को खिलाएं (अनामिक अंधे परीक्षण के लिए) और न्यायाधीश को चुनने दें कि कौन बेहतर है (ए जीतता है / बी जीतता है / समता) ।
- **Preventing Judge Biases**:
  > [! चेतावनी]
  > **The Three Sins of LLM Judges:**
  > - **Verbosity Bias**: न्यायाधीश अधिक लम्बे, अधिक शब्दार्थ उत्तरों को पसंद करते हैं।
  > - **Position Bias**: अंधे परीक्षण में, न्यायाधीश विकल्प ए (पहले स्थान पर) का चयन करने के प्रति पूर्वाग्रह रखता है।
  > - **Self-Enhancement Bias**: जीपीटी मॉडल जीपीटी शैली के आउटपुट को पसंद करते हैं, जबकि क्लाउड मॉडल क्लाउड शैली के आउटपुट को पसंद करते हैं।
  
  **Countermeasures**:
  - इस प्रश्न पत्र में स्पष्ट रूप से यह बताएं: "संक्षिप्तता एक महत्वपूर्ण आकलन है। बहुत लम्बे या ज़ोरदार उत्तरों के लिए उच्च अंक न दें।"
  - जोड़ी तुलना के लिए, ए और बी की स्थिति को आदान-प्रदान करें और औसत स्कोर लेते हुए मूल्यांकन को फिर से चलाएं।
  - मूल्यांकन किए जा रहे मॉडल से भिन्न परिवार के न्यायाधीश मॉडल चुनें (उदाहरण के लिए, जीपीटी का मूल्यांकन करने के लिए क्लॉड का उपयोग करें) ।
  - जहाँ संभव हो, 1-5 लाइकरट स्केल के बजाय द्विआधारी वर्गीकरण (पास/फेल, हाँ/नहीं) का उपयोग करें। द्विआधारी आउटपुट को फुलाया जाना अधिक कठिन है और पुनरुत्पादन करने के लिए अधिक स्थिर है।

#### स्तर 3: मानव मूल्यांकन  माप लंगर

मानव मूल्यांकन को दैनिक हजारों डेटा बिंदुओं को रेटिंग करने के लिए नहीं बनाया गया है जो किसी भी टीम को जला देगा। इसका एकमात्र उद्देश्य है**calibrating the judge model (Aligning the Judge)**. .

- **Core Implementation Practices**:
  1. **Blind Testing (Arena Mode)**: मुख्य व्यापार प्रवाह से 3050 कठिन, अस्पष्ट मामलों का चयन करें और मानव विशेषज्ञों को उन्हें अंधेरे ढंग से मूल्यांकन करने दें।
  2. **Consistency Check**: मानव ग्रेड और LLM जज ग्रेड के बीच कम्प्यूटेबल समझौते मीट्रिक (जैसे कोहेन का काप्पा) गणना करें।
- **Best Practice**:
  - यदि LLM न्यायाधीश और मानव विशेषज्ञों के बीच एकीकरण 80%85%+ तक पहुंचता है, तो न्यायाधीश को इंजीनियरिंग-विश्वसनीय माना जाता है। आप तब आत्मविश्वास से इस न्यायाधीश को 95% स्वचालित प्रतिगमन परीक्षणों को उतार सकते हैं।
  - यदि कोई गलत alignment होता है तो न्यायाधीश द्वारा दिए गए "कारण" की जांच करें और न्यायाधीश मॉडल की प्रणाली को तदनुसार शीघ्र परिष्कृत करें।
...
### ईवल डेटासेटः निर्माण और सर्वोत्तम प्रथाएं

आपके मूल्यांकन डेटासेट की गुणवत्ता आपके मूल्यांकन परिणामों की सीमा को सीधे निर्धारित करती है। मूल्यांकन डेटासेट के निर्माण में, एक पूर्ण गुणवत्ता आधार रेखा बनाने के लिए निम्नलिखित तीन प्रकार के परीक्षण मामले आवश्यक हैंः
...
उच्च गुणवत्ता वाले मूल्यांकन डेटासेट का निर्माण करने के लिए व्यवस्थित कदम और सर्वोत्तम प्रथाओं का पालन करना आवश्यक है।

#### 1. मूल्यांकन डेटासेट बनाने के लिए चार मुख्य कदम

1. **Define Evaluation Goals & Scenarios (Defining Schema)**
   "एक आकार-फिट-सभी" डेटासेट बनाने की कोशिश न करें। सबसे पहले, उन परिदृश्यों को स्पष्ट करें जिन पर आपका मॉडल लागू किया जाता हैः
   - **General Capabilities**: तर्क, कोडिंग, भूमिका-खेल, पाठ संक्षेप आदि।
   - **Vertical Business**: फिल्म मिलान, स्वचालित मेटाडेटा निष्कर्षण, एएसआर त्रुटि सुधार, लंबे वीडियो को समझने आदि।
   - **Guardrail Evaluation**: सुरक्षा, भ्रम की दर, मजबूतता (टायप और शोर की संवेदनशीलता)

2. **Determine Data Format (Data Format)**
   JSONL उद्योग में सबसे अधिक उपयोग किया जाने वाला प्रारूप है। प्रत्येक पंक्ति एक स्वतंत्र JSON ऑब्जेक्ट है, जो स्ट्रीमिंग के लिए सुविधाजनक है। मूल्यांकन विधि के आधार पर, डेटा प्रारूप आमतौर पर इस तरह संरचित हैः
...
3. **Data Sourcing & Collection (Data Sourcing)**
   - **Production Logs (Highly Recommended)**: वास्तविक उत्पादन ट्रैफ़िक (उपयोगकर्ता संकेत) से नमूना लें और इसे गुमनाम करें। यह आपके व्यवसाय के लिए सबसे यथार्थवादी "वास्तविक परीक्षण" का प्रतिनिधित्व करता है।
   - **Synthetic Data**: किसी विशिष्ट योजना के अनुसार प्रम्प्ट्स को बैच-जनरेट करने के लिए सबसे सक्षम मॉडल (जैसे, GPT-4o या Claude 3.5 Sonnet) का उपयोग करें, फिर मानव समीक्षा या हेरिस्टिक्स के माध्यम से उन्हें फ़िल्टर करें।
   - **Adoption/Modification of Public Benchmarks**: अपने विशिष्ट व्यावसायिक आवश्यकताओं के अनुरूप MMLU, GSM8K या MATH जैसे मानक बेंचमार्क अनुकूलित करें।

4. **Cleaning & Validation (Data Curation)**
   - **Deduplication**: बहुवचन रूप से दिखाई देने वाले लगभग समान संकेतों को रोकने के लिए अर्थिक समानता एल्गोरिदम या सटीक मिलान का उपयोग करें।
   - **Difficulty Grading**: विभिन्न मॉडल के बीच प्रदर्शन स्तरों को स्पष्ट रूप से अलग करने के लिए डेटासेट को आसान / मध्यम / कठिन में वर्गीकृत करें।
   - **Rule-based Filtering**: व्याकरण संबंधी त्रुटियों, अस्पष्ट वाक्यांशों, खराब रूप से परिभाषित संकेतों या संवेदनशील जानकारी को फ़िल्टर करें।

#### 2. डेटासेट निर्माण के लिए सर्वोत्तम प्रथाएं

1. **Independence & Preventing Cheating (Data Contamination Prevention)**
   - कभी भी मूल्यांकन डेटासेट के नमूने को अपने मॉडल प्रशिक्षण या सूक्ष्म-ट्यूनिंग डेटासेट में लीक होने न दें।
   - यदि मूल्यांकन सेट सार्वजनिक डेटासेट से प्राप्त होते हैं, तो उन्हें नियमित रूप से अपडेट या फिर से लिखें (जैसे, संस्थाओं को आदान-प्रदान करना, संख्याओं को बदलना, वाक्य संरचनाओं को बदलना) ताकि मॉडल को केवल याद रखने के माध्यम से उच्च स्कोर करने से रोका जा सके।

2. **Coverage & Balanced Distribution (Data Distribution)**
   - **Scenario Coverage**: यदि आपके उपयोग के मामले में "वीडियो सामग्री को समझना" शामिल है, तो आपके मूल्यांकन डेटासेट में न केवल सामान्य लंबी पाठ प्रमाणीकरण शामिल होना चाहिए, बल्कि समय स्टैम्प, विषम मेटाडेटा या लंबी संदर्भ प्रश्नों के साथ जटिल मामले भी शामिल होना चाहिए।
   - **Tag Balance**: सुनिश्चित करें कि श्रेणियों का वितरण (जैसे सारांश, निकासी, तर्क, अनुवाद) आपके वास्तविक उत्पादन यातायात के अनुरूप है।

3. **Building the "Golden Dataset" (Golden Dataset)**
   - डेटासेट आकार का अंधेरेपन से पीछा न करें। व्यावसायिक मूल्यांकन के लिए, 100500 मामलों के उच्च गुणवत्ता वाले, मैन्युअल रूप से सत्यापित "गोल्डन डेटासेट" 10,000 अशुद्ध सिंथेटिक डेटा बिंदुओं से कहीं अधिक मूल्यवान है।
   - एक स्वर्ण डेटासेट त्वरित प्रतिगमन परीक्षण को सक्षम बनाता है, जिससे आप मिनटों में मूल्यांकन चला सकते हैं और बेस मॉडल को ठीक-ठीक करने या अपग्रेड करते समय गुणवत्ता के विचलन का पता लगाते हैं।

4. **Incorporating Adversarial Testing (Adversarial Testing)**
   - डेटासेट में 5%~10% प्रतिकूल नमूने दर्ज करें।
   - उदाहरणः टाइप त्रुटियों, विरोधाभासी संदर्भ, या त्रुटियों को प्रेरित करने के लिए डिज़ाइन किए गए संकेतों के साथ इनपुट (प्रॉम्प्ट इंजेक्शन) । यह मॉडल की मजबूती और सुरक्षा सीमाओं का मूल्यांकन करने के लिए महत्वपूर्ण है।

5. **Version Control (Versioning)**
   - डेटासेट, जैसे कोड, को पुनरावृत्ति करना चाहिए। स्पष्ट संस्करण नियंत्रण लागू करें (जैसे, `eval_v1.0.jsonl`,`eval_v1.1_hard.jsonl`) ।
   - ऑनलाइन व्यापार तर्क या उपयोगकर्ता व्यवहार पैटर्न में परिवर्तन होने पर मूल्यांकन सेट को सिंक्रनाइज़ और अद्यतन करें।

### नमूना आकार और आत्मविश्वास

50 परीक्षण मामले पर्याप्त नहीं हैं।

यदि आपका मूल्यांकन 50 मामलों पर 90% (यानी 50 में से 45 सही) स्कोर करता है, तो सटीक द्विपद पद्धति (क्लोपर-पीरसन) का उपयोग करके गणना की गई 95% विश्वास अंतराल **[78%, 97%]**. इसका मतलब है कि वास्तविक सटीकता का 78% से 97% के बीच गिरने का 95% मौका है, 19 प्रतिशत अंक का प्रसार। इस नमूना आकार पर, आप सांख्यिकीय रूप से 80% की वास्तविक सटीकता के साथ एक प्रणाली को 96% के साथ अलग नहीं कर सकते।

200 मामलों में 90% सटीकता के साथ, विश्वास अंतराल को **[85%, 94%]**अब आप निर्णय ले सकते हैं।
