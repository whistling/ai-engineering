...
**BM25**(最佳匹配 25) 是标准的关键字搜索算法.它自1990年代以来一直是搜索引擎 (如Elasticsearch和Lucene) 的脊柱.它是经典的TF-IDF算法的进化升级.

如果您想快速理解BM25的运作方式,您可以专注于其**three core pillars**没有被数学公式困扰:

1. **Term Rarity (IDF: The rarer the term, the higher its weight)**其他:
   - 基本的想法是"稀有与相关性等".在查询中,几乎每个文件中都出现"什么","如何","如何"等常见术语,并不能帮助识别正确的文档.另一方面,稀有术语如"E-4021" (特定错误代码) 或"幻觉" (特定话题) 是高度选择性的.BM25自动将稀有术语和折扣赋予高重量,或忽略常见.
2. **Term Frequency Saturation (TF Saturation: Repeated hits have diminishing returns, with a ceiling)**其他:
   - 提及一项术语一次是有用的,两次是有点更有用的,但提及它100次并不使文档更相关.BM25设定了术语频率贡献的上限 (由参数控制)$k_1$) 由于一项术语在文档中出现更多次,其增量分数贡献迅速和高原.
3. **Document Length Normalization (L-Norm: Matching in short documents is more significant)**其他:
   - 核心想法是惩罚膨胀,"水"的文档.如果一个10字短标题与一个关键词匹配,而一个10,000字的综合白皮书一次匹配同一个关键词,则很可能短标题完全专注于这个关键词.BM25根据其相对长度 (由参数控制) 惩罚文档$b$),提升更短的文件,降低了极长的文件的分数.

> 现在我们要去看看.
> 详细的数学公式,平滑 IDF衍生,参数控制和步骤按步骤手动计算的数值示例,请参阅专用页面: [BM25 Mathematical Details & Code Alignment](bm25_math_en.md)现在,我们要去.


### 相互级别融合 (RRF)

在大型RAG或混合搜索架构中,**RRF (Reciprocal Rank Fusion)**是最广泛采用和最优雅的组合排名列表的算法.

简单地说,RRF的核心逻辑是:**disregard raw scores, focus only on ranks**现在,我们要去.

#### 1. 什么是我们需要RRF?
在构建混合检索系统时,我们通常使用:
- **Semantic Retrieval (Vector Search)**积分通常是共数相似性或点数,通常在$[-1, 1]$或$[0, 1]$现在,我们要去.
- **Keyword Retrieval (BM25)**: 积分是无限的正值浮点数 (取决于术语频率和文档长度).

直接总结这两个分数是数学上不准确的,因为它们的尺度,分布和物理含义完全不同.RRF通过只关注"每个列表中的文档的相对位置",将不同检索系统的输出标准化为单个维度,绕过了这种排列问题.
