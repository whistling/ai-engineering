...
在实践中的AI工程中,评估系统通常采用一个**layered architecture**通过这种设计,可确保评估的吞吐量和速度,同时控制API成本,并与人类专家一致最终判断.
...
#### 层1:基于规则和确定性 (基于规则) 必须失败-快

在调用昂贵和缓慢的LLM法官之前,首先使用确定性代码来过简单/明显的错误.

- **Core Metrics for Implementation**其他:
  - **JSON-Schema Validation**您的业务需要严格遵守特定的JSON格式 (例如标签,元数据提取),直接使用代码分析和验证.
  - **Regex & Sensitive Word Interception**: 检查输出是否包含位数 (如 `{{missing_value}}`),泄露系统提示,或包含非法/敏感的字符.
  - **Length & Type Constraints**举例来说,如果电影总结仅限于100字,如果超过150字,请标记为不符合规定.
- **Best Practice**通过安装在单元测试或CI门中,它只需要毫秒,成本为零的API,并作为一个高成本效益的安全门.

#### 层2:法官职位 现行行业工作

评估目标模型使用更有能力的模型 (建议为评审者使用第3.5条索尼特或GPT-4o).

- **Three Main Evolution Directions**其他:
  1. **G-Eval (Rubric-based Natural Language Scoring)**给法官提供一个非常详细的条款,**Chain-of-Thought (CoT)**(首先写出理由,最后点数).
  2. **QAG Mode (Question-Answer Generation Loop)**特别适合RAG (恢复增强生成) 场景. 取而代于直接评分,请问法官二进制问题:"答案完全基于参考文件吗? (是/否) ",答案是否针对用户的问题? (是/否) "转换主观问题为客观多选择/二进制任务产生高度稳定的结果.
  3. **Pairwise Preference (Head-to-Head)**:将旧提示和新提示的输出一边送给法官 (匿名用于盲测试),让法官选择哪个更好 (A获胜/B获胜/平局).
- **Preventing Judge Biases**其他:
  > 警报
  > **The Three Sins of LLM Judges:**
  > - **Verbosity Bias**评审员倾向于支持更长时间,更有文字的答案.
  > - **Position Bias**在盲测试中,法官偏向选择选项A (排名第一).
  > - **Self-Enhancement Bias**:GPT模型更喜欢GPT式输出,而克劳德模型更喜欢克劳德式输出.
  
  **Countermeasures**其他:
  - 提示书中明确说明:"简单是评估的关键标准.
  - 对于对比,换取A和B的位置,再进行评估,以平均分数.
  - 选择由评估模型不同于该模型的家庭的评审模型 (例如,使用Claude来评估GPT).
  - 尽可能使用二进制分类 (通过/失败,是/不) 而不是1-5个利克特级别.二进制输出更难膨胀,更稳定地复制.

#### 层3:人类评估 校准

人类评估不是为了每天评分数千个数据点,**calibrating the judge model (Aligning the Judge)**现在,我们要去.

- **Core Implementation Practices**其他:
  1. **Blind Testing (Arena Mode)**:从核心业务流程中选择3050个难以理解的,模糊的案例,并让人类专家盲目评分它们.
  2. **Consistency Check**计算人类成绩和法师法官成绩之间的协议指标 (如科恩的卡帕).
- **Best Practice**其他:
  - 如果法师与人体专家之间的一致性达到80%85%+,法官将被视为工程可靠.
  - 当出现错误的调整时,检查法官提供的"理由",并尽快调整法官模型的系统.
...
评估数据集的质量直接决定了评估结果的上限.在构建评估数据集时,以下三个类型的试验案例是形成完整的质量基线的必不可少:
...
建立高质量的评估数据集需要采取系统步骤并遵守最佳实践.

#### 1. 建立评估数据集的四大关键步骤

1. **Define Evaluation Goals & Scenarios (Defining Schema)**
   不要试图构建一个"适合所有人的数据集".首先,澄清您的模型应用到哪些场景:
   - **General Capabilities**推理,编码,角色扮演,文本总结等
   - **Vertical Business**电影匹配,自动地提取元数据,ASR错误纠正,长视频理解等
   - **Guardrail Evaluation**安全性,幻觉率,强度 (对字体错误和噪音敏感性).

2. **Determine Data Format (Data Format)**
   根据评估方法,数据格式通常是如下结构化:
   - **Objective Evaluation (with standard reference answers)**其他:
     ```json
     {"question": "Who directed The Matrix?", "answer": "The Wachowskis", "category": "movie_trivia"}
     ```
   - **Subjective/Open-ended Evaluation (based on LLM-as-a-Judge or human evaluation)**其他:
     ```json
...
3. **Data Sourcing & Collection (Data Sourcing)**
   - **Production Logs (Highly Recommended)**采样实际生产流量 (用户提示) 并匿名化. 这代表了您的业务最现实的"实验".
   - **Synthetic Data**:使用最有能力的模型 (例如GPT-4o或Claude 3.5 Sonnet) 来根据特定方案进行批量生成提示,然后通过人体审查或选.
   - **Adoption/Modification of Public Benchmarks**适应您的特定业务需求.

4. **Cleaning & Validation (Data Curation)**
   - **Deduplication**:使用语义相似性算法或精确匹配,以防止近似的提示出现多次.
   - **Difficulty Grading**:将数据集分为易/中/难以清楚地区分不同模型中的性能水平.
   - **Rule-based Filtering**: 选语法错误,模糊的语法,未明确的提示或敏感信息.

#### 2. 数据集建设最佳实践

1. **Independence & Preventing Cheating (Data Contamination Prevention)**
   - 永远不要让评估数据集样本泄露到模型训练或细调数据集中.
   - 如果评估集是从公共数据集中得出的,则定期更新或重写它们 (例如,交换实体,改变数字,改变句子结构),以防止模型仅通过记忆得分高.

2. **Coverage & Balanced Distribution (Data Distribution)**
   - **Scenario Coverage**: 如果您的使用案例涉及"视频内容理解",您的评估数据集不仅应包括一般长文本提示,还应包括复杂的时间标签,异质元数据或长文本查询的案例.
   - **Tag Balance**确保类别的分布 (例如总结,摘要,推理,翻译) 与实际生产流量相一致.

3. **Building the "Golden Dataset" (Golden Dataset)**
   - 对于企业评估,一个高质量的,手动验证的100500个案例的"黄金数据集"比10,000个未经清洁的合成数据点要多得多.
   - 黄金数据集可实现快速回归测试,允许您在几分钟内进行评估,并在调整或升级基模型时检测质量波动.

4. **Incorporating Adversarial Testing (Adversarial Testing)**
   - 输入5%~10%的对抗样本到数据集中.
   - 举个例子:输入具有字体错误,冲突的背景或旨在诱导错误的提示 (Prompt Injections).这对于评估模型强度和安全界限至关重要.

5. **Version Control (Versioning)**
   - 数据集,如代码,必须进行反复. 实现清晰的版本控制 (例如,`eval_v1.0.jsonl`现在`eval_v1.1_hard.jsonl`)
   - 当在线商业逻辑或用户行为模式发生变化时,同步和更新评估组.

### 样本规模和自信

五个试验案例不够.

如果你的评分在50个案例中达到90% (即50个案例中45个正确),则使用精确二项方法 (Clopper-Pearson) 计算的95%的信任间隔是**[78%, 97%]**这意味着真正的精度有95%的机会降低到78%至97%,差距为19个百分点.在这个样本大小下,你无法统计地区分一个具有80%的确度的系统与一个具有96%的系统.

在200例中,90%的准确度,信任间隔缩小到**[85%, 94%]**现在你可以做出决定.
