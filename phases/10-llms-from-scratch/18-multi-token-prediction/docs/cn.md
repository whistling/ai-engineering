...
**Prerequisites:** Phase 10 · 04 (预训练一个迷你 GPT), Phase 10 · 15 (投机性解码)  
**Time:** ~60 分钟

## Learning Objectives

- 说明 MTP 的训练目标并推导跨预测深度的联合损失。
- 解释 Gloeckle et al. 的并行 MTP 头（2024）与 DeepSeek-V3 的顺序 MTP 模块之间的差异，以及为什么顺序设计能保留因果链。
- 计算在预训练运行中添加 MTP 模块的参数和内存开销。
- 从零实现一个 MTP 模块：共享嵌入、逐深度的 transformer 块、投影和共享输出头。
...
| Backward-compatible with existing weights | Yes | No（需要重新训练） |
| Draft params | 1-2 transformer layers | 1 transformer block + projection |
| Acceptance rate | 0.88-0.92 | 0.80+ at depth 1 |
| Benefit beyond speedup | Speculative decoding only | 更密集的训练信号 + 加速 |
...
def combine(prev_hidden, next_token_embed, M_k):
    # 在特征维上连接，然后投影回隐藏维
    concat = rms_norm(prev_hidden) + rms_norm(next_token_embed)  # 用向量加法替代连接以简化
...
| 术语 | 常说法 | 实际含义 |
|------|--------|---------|
| MTP module | “Extra loss block” | 一个小的 transformer 块加上投影，用于预测主模型在位置 `i` 后第 `k` 个 token |
| Prediction depth | “Which offset” | 整数 `k`，表示模块 `k` 预测 `t_{i+k}`（基于前缀到位置 `i`） |
| Parallel MTP | “Gloeckle-style” | D 个独立输出头作用于同一 backbone 隐状态，没有条件链 |
| Sequential MTP | “DeepSeek-V3 style” | 每个模块基于前一深度的隐藏状态加上下一个 token 的嵌入进行条件化；保留因果链 |
| Shared output head | “Reuse the main head” | MTP 模块调用主模型的 LM head，而不是独立的输出投影 |
| Shared embedding | “Reuse the main table” | 使用相同的词表嵌入表；没有重复参数 |
| Projection matrix M_k | “Combine hidden + next-token” | 一个 `h x 2h` 的线性层，将前一隐藏态和目标 token 嵌入折叠为下一深度的输入 |
| Joint loss L_MTP | “Averaged extra losses” | 每深度交叉熵损失的算术平均，并乘以缩放因子 `lambda` |
| Acceptance rate at depth 1 | “How often MTP draft is right” | D=1 时 MTP 模块的 top-1 预测与主模型 top-1 预测相等的比率；DeepSeek-V3 上为 80%+ |
| Lambda weighting | “Extra-loss importance” | 额外损失的重要性缩放因子；DeepSeek-V3 在训练早期使用 0.3，后期使用 0.1 |
