...
| 维度 / 模式 | ReAct | ReWOO | Plan-and-Execute | Plan-and-Act |
| :--- | :--- | :--- | :--- | :--- |
| **规划时机** | 运行时（每一步都规划） | 编译期（一次性规划完） | 运行前规划 + 中途修正 | 运行前高层规划 + 运行时微调 |
| **LLM 介入频率** | 极高 (每步必调用) | 极低 (通常仅 2 次) | 中等 (取决于重规划频率) | 高 (每步执行仍需大模型介入) |
| **并发能力** | 差 (必须串行) | 极强 (互不依赖可并发) | 中等 (阶段内可并发) | 差 (通常依赖实时反馈) |
| **中途纠错能力** | 极强 | 无 (只能靠 Solver 硬顶) | 强 (宏观层面的掉头) | 强 (微观层面的调整) |
| **Token 消耗** | 极高 | 极低 | 适中 | 较高 |
| **适用场景** | 短链路、交互性强、高度不可预测的任务 | 高度确定性、数据密集型、需要高吞吐的流水线 | 长周期、复杂的多步骤探索性任务 | 目标明确但中间执行细节多变的探索性任务 |
