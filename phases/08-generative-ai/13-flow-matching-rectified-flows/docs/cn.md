...
|------|-----------------|-----------------------|
| Flow matching | "Straight-line diffusion" | 训练 `v_θ(x, t)` 去匹配沿插值的 `x_1 - x_0`。 |
| Rectified flow | "Reflow" | 迭代地将学习到的流线拉直的过程。 |
| Velocity field | "v_θ" | 模型的输出 — 表示移动 `x_t` 的方向。 |
| Straight-line interpolant | "The path" | `x_t = (1-t)·x_0 + t·x_1`; 简单的目标导数。 |
| Euler sampler | "1st order ODE solver" | 最简单的积分器；当路径足够直时效果很好。 |
| Logit-normal t | "SD3 sampling" | 将 `t` 采样集中到梯度最强的中间值。 |
| Consistency distillation | "1-step sampler" | 训练学生模型直接将任意 `x_t` 映射到 `x_0`。 |
