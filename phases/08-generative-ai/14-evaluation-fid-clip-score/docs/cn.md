...
|------|-----------------|-----------------------|
| FID | "Fréchet Inception Distance" | 对真实与生成 Inception 特征做高斯拟合后的 Fréchet 距离。 |
| CLIP score | "Text-image similarity" | CLIP 图像嵌入与文本嵌入的余弦相似度。 |
| CMMD | "FID's replacement" | 基于 CLIP 特征的 MMD（最大均值差异）；偏差更小，无高斯假设。 |
| IS | "Inception score" | Exp KL(p(y|x) || p(y))；在现代模型上相关性差，已逐步弃用。 |
| HPSv2 / ImageReward / PickScore | "Learned preference proxies" | 在人类偏好上训练的小模型；用作自动判别器。 |
| Elo | "Chess rating" | 基于成对胜负的 Bradley–Terry 聚合评分。 |
| PartiPrompts | "The benchmark prompt set" | Google 策划的 1,600 条提示，覆盖 12 个类别。 |
| FD-DINO | "Self-sup replacement" | 使用 DINOv2 特征的 FD；对非 ImageNet 域更好。 |
